...
def make_train(config, env, test_env, test_env_modif, meta_policy, meta_policy_llm, renderer):
    ...
    config["TEST_NUM_UPDATES"] = config.get("TEST_UPDATE_TIMESTEPS", 0) // config["NUM_STEPS"] // config["TEST_NUM_ENVS"] if config.get("TRAIN_ON_MODIF", False) else 0
    ...
    def train(rng, params, batch_stats):
        ...
        # TRAINING LOOP
        def _update_step(runner_state, only_meta_update):
            ...
            metrics, train_states = jax.lax.cond(
                jnp.logical_or(only_meta_update, config.get("FREEZE_AGENTS", False)),
                lambda: jax.vmap(fake_update_agent, in_axes=(0, 0, 0, None))(train_states, jnp.arange(num_agents), rngs, network),
                lambda: jax.vmap(_update_agent, in_axes=(0, 0, 0, None))(train_states, jnp.arange(num_agents), rngs, network),
            )
        ...
        def test_and_log(metrics, runner_state):
            ...
            if config.get("TEST_DURING_TRAINING", False):
                ...
                test_metrics = jax.lax.cond(
                    ...
                    lambda _: get_test_metrics(train_states, meta_train_state, False, False, _rng),
                    lambda _: test_metrics,
                    operand=None,
                )
                inner_metrics.update({f"test/{k}": v for k, v in test_metrics.items()})
                if config.get("TEST_MODIFS", False):
                    rng, _rng = jax.random.split(rng)
                    test_metrics_modif = jax.lax.cond(
                        train_states.n_updates[0] % int(config["NUM_UPDATES"] * config["TEST_INTERVAL"]) == 0,
                        lambda _: get_test_metrics(train_states, meta_train_state, True, True, _rng),
                        lambda _: test_metrics_modif,
                        operand=None,
                    )
                    inner_metrics.update({f"test_modif/{k}": v for k, v in test_metrics_modif.items()})
            ...
            #     only_meta_update,
            #     lambda: (rng, test_metrics, test_metrics_modif), #do not test or log if only meta update
            #     lambda: _test_and_log(rng, test_metrics, test_metrics_modif),
            # )
        ...
        def get_test_metrics(train_states, meta_train_state, vmap_reset_fn, vmap_step_fn, label, train_on_test, rng):
            ...
            if config.get("TRAIN_ON_MODIF", False) and train_on_test:
                ...
                _, meta_train_state, _, _, _ = runner_state
        ...
        def _update_and_metrics(runner_state, only_meta_update):
            ...
            rng, step_metrics, test_metrics = test_and_log(step_metrics, runner_state)
            runner_state = (runner_state[0], runner_state[1], runner_state[2], test_metrics, rng)
        ...
        for vmap_reset_fn, vmap_step_fn, label in eval_closures:
            ...
            env_metrics = get_test_metrics(
                train_states, meta_train_state, vmap_reset_fn, vmap_step_fn, label, False, __rng
            )
