...
def eps_greedy_exploration(rng, q_vals, eps):
    """Epsilon-greedy action selection (module level so it can be reused offline)."""
...
def compute_meta_q_vals(
    ...
    """Score the options with the meta-policy.

    Single source of truth for how the meta Q-values are formed: the training
    rollout, the evaluation rollout and the offline `eval_activations.py`
    script all go through this function, so they cannot drift apart.

    `n_updates` is the option networks' update counter, used only by the
    pretraining schedules; `rng_rand` is only consumed by RANDOM_PRETRAIN.
    """
    ...
    def handle_conditional():
        active_agent_q_vals = meta_policy(
            meta_network, meta_train_state, last_obs, env_state
        )
    ...
    def handle_random_pretrain():
        ...
        random_q = jax.random.randint(
            rng_rand, shape=both_q.shape, minval=0, maxval=both_q.shape[-1]
        ).astype(jnp.float32)
        return jax.lax.cond(
            n_updates < config["PRETRAIN_LEN"],
            lambda _: random_q,
...
def select_active_agent(config, combined_q, num_envs, eps, rng):
    """Pick the option to execute from the meta Q-values (greedy or eps-greedy)."""
    ...
    def select_exploratory(_):
        rng_split = jax.random.split(rng, num_envs)
        eps_per_env = jnp.full((num_envs,), eps)
        return jax.vmap(eps_greedy_exploration)(rng_split, combined_q, eps_per_env)
...
def make_train(config, env, eval_envs, meta_policy, meta_policy_llm, renderer):
    ...
    # Build eval closures: (vmap_reset, vmap_step, label) per eval env
    eval_closures = tuple(
        (lambda n_envs, _env=env_obj: lambda rng: jax.vmap(_env.reset)(jax.random.split(rng, n_envs)),
         lambda env_state, action, _env=env_obj: jax.vmap(_env.step)(env_state, action),
         label)
        for env_obj, label in eval_envs
    )
    ...
    def train(rng, params, batch_stats):
        ...
        def _update_step(runner_state, unused):
            ...
            def _step_env(carry, _):
                ...
                combined_q = compute_meta_q_vals(
                    config,
                    meta_policy,
                    meta_policy_llm,
                    meta_network,
                    ...
                    env_state,
                    all_q_vals,
                    train_states.n_updates[0],
                    rng_a,
                )

                # Now select active agent — greedy vs exploratory
                active_agent = select_active_agent(
                    config,
                    combined_q,
                    config["NUM_ENVS"],
                    eps_meta_scheduler(meta_train_state.n_updates),
                    rng_s,
                )
            ...
            if config.get("TEST_DURING_TRAINING", False):
                ...
                def _do_eval(rng):
                    results = []
                    for vmap_reset_fn, vmap_step_fn, label in eval_closures:
                        rng, _rng = jax.random.split(rng)
                        env_metrics = get_test_metrics(
                            train_states, meta_train_state, vmap_reset_fn, vmap_step_fn, label, _rng
                        )
                        results.append(env_metrics)
                    return rng, tuple(results)

                def _skip_eval(rng):
                    return rng, test_metrics

                rng, test_metrics = jax.lax.cond(
                    should_eval,
                    _do_eval,
                    _skip_eval,
                    _rng,
                )
                for i, (_, _, label) in enumerate(eval_closures):
                    env_metrics = test_metrics[i]
                    metrics.update(
                        {f"test_{label}/{k}": v for k, v in env_metrics.items()}
                    )
        ...
        def get_test_metrics(train_states, meta_train_state, vmap_reset_fn, vmap_step_fn, label, rng):
            ...
            def _env_step(carry, _):
                ...
                new_obs, new_env_state, reward, done, info = vmap_step_fn(
                    env_state, action
                )
            ...
            rng, _rng = jax.random.split(rng)
            init_obs, env_state = vmap_reset_fn(config["TEST_NUM_ENVS"])(_rng)
            ...
            if config.get("RECORD_VIDEO", False):
                jax.lax.cond(
                    ...
                    lambda _: jax.debug.callback(video_callback, states, active_agents, combined_qs, dones, train_states.n_updates[0], renderer, label=label),
                    lambda _: None,
                    operand=None,
                )
        ...
        eval_init_results = []
        for vmap_reset_fn, vmap_step_fn, label in eval_closures:
            rng, __rng = jax.random.split(rng)
            env_metrics = get_test_metrics(
                train_states, meta_train_state, vmap_reset_fn, vmap_step_fn, label, __rng
            )
            eval_init_results.append(env_metrics)
        test_metrics = tuple(eval_init_results)
