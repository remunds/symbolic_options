...
The grid is 2x3: the three skill-return panels per game (no environment-return
column).

Unlike ``new_reward_plot_new_runs.py``, the three shaped-reward ablations
(``pqn_shapedreward*`` from ``NEXUS_rebuttal2``) are intentionally omitted.
...
# Concatenate NEXUS + PPO rows into the same per-game dataframe.
seaquest_df = pd.concat([ppo_seaquest_df, nexus_seaquest_df], ignore_index=True)
kangaroo_df = pd.concat([ppo_kangaroo_df, nexus_kangaroo_df], ignore_index=True)
...
def multi_reward_plot(df, opt_names, axes):
    ...
    if "craft" in game_name:
        ...
        algos = [f"ppo_{game_name}", f"pqn_{game_name}", f"pqn_hier_baseline_{game_name}", f"pqn_hier_{game_name}", f"pqn_hier_llm_{game_name}", f"pqn_hier_comb_{game_name}"]
