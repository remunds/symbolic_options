"""Skill-reward plots for JAXAtari Seaquest and Kangaroo with additional runs.

This is a standalone copy of the ``multi_reward_plot`` setup from
``plots.ipynb`` (cell 12): the data loader ``get_run_data``, the per-skill
curve plotter ``single_reward_plot`` and the grid ``multi_reward_plot``.
...
Data sources:
  * Previous (JAXAtari) runs: ``raban-emunds-tu-darmstadt/LENS`` -- the exact
    same run ids / project as in ``plots.ipynb``.
  * Additional runs: ``raban-emunds-tu-darmstadt/NEXUS_rebuttal2`` -- three flat
    PQN + shaped-reward ablations per game:
...
The old and the new rows are concatenated into the same per-game dataframe, so
the new ablations are drawn as extra (dashed) skills curves in every panel.

Produces:
    new_reward_plot_seaquest_kangaroo.pdf
    new_reward_plot_seaquest_kangaroo.svg
"""
...
seaquest_runs = {
    run_id: f"raban-emunds-tu-darmstadt/LENS/runs/{run_hash}"
    for run_id, run_hash in seaquest_run_ids.items()
}
old_seaquest_df = get_run_data(seaquest_runs)
...
kangaroo_runs = {
    run_id: f"raban-emunds-tu-darmstadt/LENS/runs/{run_hash}"
    for run_id, run_hash in kangaroo_run_ids.items()
}
old_kangaroo_df = get_run_data(kangaroo_runs)

# plots.ipynb cell 7: Seaquest's comb runs were named '*_shootdefault'; strip
# the suffix so they merge into plain 'pqn_hier_comb_seaquest'.
old_seaquest_df["run_name"] = old_seaquest_df["run_name"].str.replace("_shootdefault", "", case=False)

# ---------------------------------------------------------------------------
# Additional runs (NEXUS_rebuttal2, flat PQN + shaped reward ablations)
# ---------------------------------------------------------------------------
seaquest_new_run_ids = {
    "shaped": "i9qzgdge",
    "shaped_simple": "w9dw1bgk",
    "shaped_llm": "1hkxcsdn",
}
kangaroo_new_run_ids = {
    "shaped": "80bgyq4o",
    "shaped_simple": "vxx1sdvy",
    "shaped_llm": "8sgqi5ru",
}
NEW_PROJECT = "raban-emunds-tu-darmstadt/NEXUS_rebuttal2"
seaquest_new_runs = {
    run_id: f"{NEW_PROJECT}/runs/{run_hash}" for run_id, run_hash in seaquest_new_run_ids.items()
}
kangaroo_new_runs = {
    run_id: f"{NEW_PROJECT}/runs/{run_hash}" for run_id, run_hash in kangaroo_new_run_ids.items()
}
new_seaquest_df = get_run_data(seaquest_new_runs)
new_kangaroo_df = get_run_data(kangaroo_new_runs)

# Concatenate old + new rows into the same per-game dataframe.
seaquest_df = pd.concat([old_seaquest_df, new_seaquest_df], ignore_index=True)
kangaroo_df = pd.concat([old_kangaroo_df, new_kangaroo_df], ignore_index=True)
...
with plt.rc_context({
    **bundles.neurips2024(usetex=False),
    ...
    fig.tight_layout()
    fig.savefig(OUT_DIR / "new_reward_plot_seaquest_kangaroo.pdf", bbox_inches="tight")
    fig.savefig(OUT_DIR / "new_reward_plot_seaquest_kangaroo.svg", bbox_inches="tight")
    plt.close(fig)
