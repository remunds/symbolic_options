...
We now provide a `uv`-based setup path for Python and dependency management.
...
uv sync --extra jaxatari
```

JAXAtari requires Atari sprites. After dependency installation, run:

```bash
install_sprites
```

To install the legacy dependency stack from this repository's pinned list:
...
- **JAXAtari Seaquest**: JAXAtari-backed RM tasks over Seaquest. Use `--env=Jax-Seaquest-RM1-v0` or `--env=Jax-Seaquest-RM2-v0`.
...
Example command for JAXAtari Seaquest:

```bash
uv run python reward_machines/run.py --env=Jax-Seaquest-RM1-v0 --alg=deepq --num_timesteps=1e6
```
