...
def build_env(args):
    ...
    if alg in ['deepq', 'deepq_clean', 'qlearning', 'hrm', 'dhrm']:
        env = make_env(env_id, env_type, args, seed=seed, logger_dir=logger.get_dir())
    else:
        # Lazily import TensorFlow only for algorithms that require it.
        try:
            import tensorflow as tf
            from baselines.common.tf_util import get_session
        except ImportError as ex:
            raise ImportError(
                'Algorithm {} requires TensorFlow and baselines tf utilities. '
                'Install TensorFlow or use run.py for TensorFlow-based setups.'.format(alg)
            ) from ex
